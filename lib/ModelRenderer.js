...
class ModelRenderer {
    ...
    _setupInternals() {
        const pickOut = (oNode) => {
            ...
            if (oNode.name !== "struct" || oNode.name !== "constraints") {
                oNode.children.forEach(oChild => pickOut(oChild));
            }
        };
        pickOut(this._model);
    }
    ...
    _traverse(node, level = 0, childIndex = -1) {
        ...
        if (ModelRenderer.contains(node.attributes, "mandatory")) {
            if (level === 1) {
                node.selected = true;
            } else {
                let par = node.parent;
                while (par && ModelRenderer.contains(par.attributes, "abstract")) {
                    par = par.parent;
                }
        ...
        if (typeof node.selectable === "undefined") {
            node.selectable = ModelRenderer.contains(node.attributes, "abstract") ? false : true;
        }
        ...
        output = ModelRenderer.contains(node.attributes, "abstract", undefined) ? colors.grey(featureName) : output;
    ...
    _markFeature(node, sFeatureName) {
        if (ModelRenderer.contains(node.attributes, "name", "") === sFeatureName && node.selectable) {
            node.selected = !node.selected;
            if (node.parent && node.parent.name === "alt") {
                node.parent.children
                    .filter(child => ModelRenderer.contains(node.attributes, "name", "") !== ModelRenderer.contains(child.attributes, "name", ""))
    ...
    _getMarkedFeatures(node, bTreeReached) {
        if (node.selected && !ModelRenderer.contains(node.attributes, "abstract", false)) {
            return [
    ...
    showLegend() {
        ...
        this.addAdditionalWritenLinesToClear(7);
    }
